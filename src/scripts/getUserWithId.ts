...
/**
 * This function is responsible for returning a user's information by id from the remote server.
 *
...
const getUserWithId = async (user: string): Promise<AuthType | {}> => {
  let userData: AuthType | any = {};

  await api.instance
    .get(`${api.endpoints.getUserInfoWithId}?key=${api.apiKey}&user=${user}`)
